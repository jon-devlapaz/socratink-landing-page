...
void main()
{
  ...
  vec3 inkColor = uBaseColor;
  inkColor = mix(inkColor, uLightAColor, lightAIntensity * fresnel * 0.85);
  inkColor = mix(inkColor, uLightBColor, lightBIntensity * fresnel);
  inkColor = mix(inkColor, uLightCColor, lightCIntensity * fresnel * 0.55);
  ...
  inkColor += uSpecularColor * (specA * 1.2 * keyMask + specB * 1.05 * rimMask + specC * 0.32 * fillMask);

  float hot = clamp(pow(max(0.0, fresnel - 0.74), 2.6), 0.0, 1.0) * uHotRim * rimMask;
  inkColor = mix(inkColor, vec3(1.0), hot);
