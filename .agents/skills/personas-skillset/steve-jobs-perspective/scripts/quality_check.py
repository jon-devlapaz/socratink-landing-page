...
示例:
    python3 quality_check.py /Users/jondev/.agents/skills/personas/elon-musk-perspective/SKILL.md
"""
...
def check_expression_dna(content: str) -> tuple[bool, str]:
    ...
    style_markers = len(re.findall(
        ...
        r'sentence|vocabulary|tone|humor|rhythm|certainty|citation|catchphrase',
        content,
        re.IGNORECASE,
    ))
...
def check_primary_sources(content: str) -> tuple[bool, str]:
    ...
    source_section = re.search(
        r'(?:##\s+.*来源|##\s+(?:Research\s+)?Sources?|##\s+Reference)(.*?)(?=\n##\s|\Z)',
        content,
    ...
    # English skills commonly group source links under explicit subsections.
    # Count the list entries rather than repeated marker words.
    ...
    secondary_section = re.search(
        r'###\s+(?:Secondary|External)[^\n]*\n(.*?)(?=\n###\s|\Z)',
        source_text,
        re.DOTALL | re.IGNORECASE,
    )
    if primary_section or secondary_section:
        primary = len(re.findall(r'^[-*]\s+', primary_section.group(1), re.MULTILINE)) if primary_section else 0
        secondary = len(re.findall(r'^[-*]\s+', secondary_section.group(1), re.MULTILINE)) if secondary_section else 0
